...
class ACH extends LocalGateway {
	...
	protected function init() {
		...
		add_filter( 'fkwcs_localized_data', array( $this, 'localize_element_data_ach' ), 999 );
	 ...
	 * @param bool $save_payment_method Force payment source to be saved.
	 *
	 * @return array|void
	 * @throws \Exception If payment will not be accepted.
	 */
	public function process_payment( $order_id, $retry = true, $save_payment_method = false, $previous_error = false, $use_order_source = false ) { //phpcs:ignore VariableAnalysis.CodeAnalysis.VariableAnalysis.UnusedParameter,VariableAnalysis.CodeAnalysis.VariableAnalysis.UnusedVariable
	...
	public function process_payment_using_saved_token( $order_id ) {
		...
		try {
			...
			$request = array(
				...
				'description'            => $this->get_order_description( $order ),
				'customer'               => ! empty( $payment_method->customer ) ? $payment_method->customer : $this->get_customer_id( $order ),
				'expand'                 => array(
					'payment_method',
				...
				try {
					...
					switch ( $confirm_response['data']['status'] ) {
						...
						case 'processing':
							...
							/* translators: 1: Intent ID, 2: Additional information */
	...
	/**
	 * Persist an ACHToken when the customer changes a subscription's payment method.
	 *
	 ...
	 * card gateway, ACH has no runtime Stripe->WC token sync, so without this the new bank
	 * never appears under My Account -> Payment methods. We hook the change-payment success
	 * action and create the token for `us_bank_account` payment methods only.
	 *
	...
	public function maybe_save_token_on_change_payment_method( $source_id = '', $prepared_source = null ) { //phpcs:ignore VariableAnalysis.CodeAnalysis.VariableAnalysis.UnusedParameter
		try {
			...
			/** Only ACH bank accounts - the action fires for every gateway's change-payment flow. */
			if ( ! isset( $payment_method->type ) || 'us_bank_account' !== $payment_method->type ) {
				return;
			}
			...
			Helper::log( sprintf( 'ACH change-payment tokenization failed: %s', $e->getMessage() ), 'error' );
		}
	}
	...
	public function payment_fields() {
		...
		echo '<div id="fkwcs_stripe_ach-payment-data">';
	 ...
	 * @param object    $intent Stripe payment intent object.
	 *
	 * @return void
	 */
	public function save_payment_method( $order, $intent ) {
		...
		$stripe_api     = $this->get_client();
		$response       = $stripe_api->payment_methods( 'retrieve', array( $payment_method ) );
		$payment_method = $response['success'] ? $response['data'] : false;

		if ( $payment_method && empty( $payment_method->customer ) ) {
			$customer_id = $this->get_customer_id( $order );
			if ( ! empty( $customer_id ) ) {
				$attach_response = $stripe_api->payment_methods( 'attach', array( $payment_method->id, array( 'customer' => $customer_id ) ) );
				if ( ! $attach_response['success'] ) {
					Helper::log( sprintf( 'Failed to attach payment method %1$s to customer %2$s: %3$s', $payment_method->id, $customer_id, $attach_response['message'] ?? 'Unknown error' ), 'warning' );
				}
			}
		}
	...
	public function localize_element_data_ach( $data ) {
	...
	public function get_element_options() {
		/** Same unit mismatch as the parent: compare in minor units, format only once. */
